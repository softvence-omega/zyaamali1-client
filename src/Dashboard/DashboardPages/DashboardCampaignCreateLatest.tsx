...
const DashboardCampaignCreateLatest = () => {
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      <p className="text-sm text-gray-600">Hint: Choose your campaign goal, platform, and basic setup.</p>
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {objectives.map((objective, index) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-center">{objective.label}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {(adTypesMap[selectedPlatform] || []).map((adType, index) => (
            ...
            >
              ...
              <span className="text-xs text-center text-gray-700">{adType.label}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <p className="text-sm text-gray-600">Hint: Define who sees your ad based on demographics and interests.</p>
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      <p className="text-sm text-gray-600">Hint: Set your spending limits and when the campaign runs.</p>
      <div className="space-y-3">
        ...
        {!lifetimeBudget ? (
          ...
          <div>
            <p className="text-sm">Lifetime Budget</p>
            <Input
              type="text"
              value={totalBudget}
              required
...
const renderStep4 = () => (
  <div className="space-y-6 ">
    <p className="text-sm text-gray-600">Hint: Create your ad content, upload media, and preview how it looks.</p>
    ...
    <div>
      ...
      <div className="mb-4 mt-5">
        ...
        <input
          ...
          className="border border-gray-200 rounded-lg w-full py-2 px-3 mt-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
        />
      </div>
      <div className="mb-4 mt-5 flex flex-col">
        ...
        <select
          ...
          className="py-2 px-2 border border-gray-200 rounded-lg mt-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
        >
      ...
      <div>
        ...
        <textarea
          ...
          className="border-[1px] border-gray-200 w-full rounded-xl px-3 py-2 text-sm mt-2 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
          rows={4}
        />
        {errors.prompt && (
    ...
    <div className="mt-5">
      ...
      {/* <div className="space-y-4">
        <div>
          <label className="text-sm font-medium">Upload Video (max 10MB)</label>
          <Input
            type="file"
        ...
        <div>
          <label className="text-sm font-medium">Upload Carousel Images (max 5MB each)</label>
          <Input
            type="file"
            accept="image/jpeg,image/png"
            multiple
          ...
          {carousel && (
            <p className="text-sm text-gray-600 mt-1">
              Selected: {Array.from(carousel).map(file => file.name).join(", ")}
            </p>
          )}
        </div>
      ...
      <div className="bg-black rounded-md flex justify-center items-center gap-2 h-64 mx-6">
        {image ? (
          ...
          <p className="text-white">No media uploaded</p>
        )}
      </div>
      <div className="flex justify-center items-center py-3 gap-4">
        <button
          onClick={saveDraft}
          className="px-4 py-2 rounded-3xl border border-[#8E6EFF] text-[#8E6EFF] font-bold hover:bg-[#F4F1FF]"
        >
          Save Draft
        </button>
        <button
    ...
    <div className="flex items-center w-full p-3 rounded-full bg-[#f5f3ff] shadow-sm">
      <div className="relative text-blue-500">
        <select
          className="w-[100px] py-4 px-2 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
          onChange={(e) => console.log("Selected prompt type:", e.target.value)} // Placeholder for future functionality
        >
      ...
      <input
        ...
        onChange={(e) => console.log("Prompt input:", e.target.value)} // Placeholder for future functionality
      />
    ...
    <div className="mt-5 ">
        ...
        <div className="inline-flex items-center bg-white border border-gray-300 rounded-full overflow-hidden text-sm font-medium">
          <button
            ...
            className={`px-4 py-3 transition-colors duration-200 ${
              selected === "mobile" ? "bg-purple-500 text-white" : "text-black"
            }`}
          >
            Mobile
          </button>
          <button
            ...
            className={`px-4 py-3 transition-colors duration-200 ${
              selected === "desktop" ? "bg-purple-500 text-white" : "text-black"
            }`}
          >
            Desktop
          </button>
      ...
      <div className="mt-7 flex justify-center">
        {selected === "mobile" ? (
          <div className="border-8 border-gray-800 rounded-3xl p-4 max-w-xs">
            {image ? (
              <img
                src={URL.createObjectURL(image)}
                alt="Mobile Preview"
              ...
              <p className="text-gray-600">No media uploaded for preview</p>
            )}
            <p className="mt-2 text-sm font-bold">{title}</p>
            {selectedPlatform === "Google" && headline.filter(h => h).length > 0 && (
              <p className="text-xs text-gray-600">{headline.filter(h => h)[0]}</p>
            )}
          </div>
        ) : (
          <div className="border border-gray-300 p-4 max-w-2xl">
            {image ? (
              <img
                src={URL.createObjectURL(image)}
                alt="Desktop Preview"
            ...
            <p className="mt-2 text-sm font-bold">{title}</p>
            {selectedPlatform === "Google" && headline.filter(h => h).length > 0 && (
              <p className="text-xs text-gray-600">{headline.filter(h => h).join(" | ")}</p>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="p-6 bg-[#F3F4F6] rounded-[20px]">
      <p className="text-lg">
