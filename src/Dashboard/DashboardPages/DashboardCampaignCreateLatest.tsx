  ...
  const handleSubmit = async () => {
    ...
    if (selectedPlatform === "Facebook") {
      ...
      payload = {
        ...
        adGroupName,
        adType: selectedAdType,
        budgetAmountMicros: lifetimeBudget
          ? parseInt(totalBudget) * 1000000
          : parseInt(dailyBudget) * 1000000,
        cpcBidMicros: 800000,
      ...
      <div>
        ...
        <div className="flex justify-between">
          {platforms.map((platform, index) => (
            ...
            >
              <platform.icon
                className={`text-2xl ${platform.color} dark:text-gray-300`}
              />
            </div>
          ))}
        </div>
  ...
  const renderStep5 = () => (
    <div className="space-y-6">
      <p className="text-sm text-gray-600">
        Hint: Review all details before launching.
      </p>
      <div>
