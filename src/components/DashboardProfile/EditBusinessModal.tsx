...
const EditBusinessModal: React.FC<EditBusinessModalProps> = ({ isOpen, onClose, business, onUpdate }) => {
    // Initialize react-hook-form with default values from the 'business' prop
    const { register, handleSubmit, reset, formState: { errors } } = useForm<EditBusinessFormInputs>({
        defaultValues: {
            name: business.name,
    ...
    return (
        <div className="fixed inset-0  bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-auto">
                <h2 className="text-xl font-semibold text-gray-800 mb-4">Edit Business</h2>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <div>
                        <label htmlFor="editBusinessName" className="block text-sm font-medium text-gray-700">
                            Business Name
                        </label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label htmlFor="editBusinessDescription" className="block text-sm font-medium text-gray-700">
                            Description
                        </label>
                        <textarea
                    ...
                    <div>
                        <label htmlFor="editBusinessGoal" className="block text-sm font-medium text-gray-700">
                            Goal
                        </label>
                        <textarea
                            id="editBusinessGoal"
                            rows={2}
                            className={`mt-1 block w-full px-3 py-2 border ${errors.goal ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm`}
